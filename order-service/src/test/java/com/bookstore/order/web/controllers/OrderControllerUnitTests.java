...
public class OrderControllerUnitTests {
    ...
    @BeforeEach
    void setUp() {
        given(securityService.getLoginUserName()).willReturn("dang");
    }

    @ParameterizedTest(name = "[{index}]-{0}")
    @MethodSource("createOrderRequestProvider")
    void shouldReturnBadRequestWhenOrderPayloadIsInvalid(CreatedOrderRequest request) throws Exception {
        given(orderService.createOrder(eq("dang"), any(CreatedOrderRequest.class)))
                .willReturn(null);
